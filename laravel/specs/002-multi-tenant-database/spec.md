...
- A tenant database creation fails mid-provisioning (disk full, permission, migration error): the system MUST NOT leave a half-created tenant in an "active" state; the central tenant record either rolls back or is flagged as failed and surfaced to the administrator with the underlying error, and the hostname is freed.
...
- **FR-004**: Creating a tenant MUST be a single atomic operation from the user's perspective: it MUST create the central tenant record, provision a new database, run the tenant migrations into it, and bind at least one unique hostname to the tenant; if any step fails, the system MUST NOT leave the tenant in an "active" state and MUST surface the failure to the administrator.
...
- **Tenant**: An isolated customer (a school). Attributes: unique identifier, unique slug, display name, status (active / soft-deleted), creation timestamp, soft-deletion timestamp (nullable), reference to its dedicated database. Lives in the central database. Soft-delete only; never hard-deleted via the standard flow.
...
- **SC-001**: A central administrator can create a brand new tenant — including database creation, tenant migrations, hostname binding — in under 60 seconds end-to-end through the admin interface, with no manual database steps.
